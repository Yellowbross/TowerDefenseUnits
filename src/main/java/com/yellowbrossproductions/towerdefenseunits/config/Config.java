...
public class Config {
    ...
    static {
        TowerDefenseConfig.init(common_builder, client_builder);
    ...
    public static void loadConfig(ForgeConfigSpec config, String path) {
        TowerDefenseUnits.LOGGER.info("Loading config: " + path);
        final CommentedFileConfig file = CommentedFileConfig.builder(new File(path)).sync().autosave().writingMode(WritingMode.REPLACE).build();
        TowerDefenseUnits.LOGGER.info("Built config: " + path);
        file.load();
        TowerDefenseUnits.LOGGER.info("Loaded config: " + path);
